...
TOK = [
    ...
    ('A_{F}', r'$A_{\mathrm{F}}$'), ('B_{F}', r'$B_{\mathrm{F}}$'),
    ('R_{BE}', r'$R_{\mathrm{BE}}$'),
    ('\u03a3', r'$\Sigma$'), ('\u03be', r'$\xi$'), ('\u03b3', r'$\gamma$'),
    ('\u03bb', r'$\lambda$'), ('\u03b6', r'$\zeta$'), ('\u03c6', r'$\phi$'),
]
...
b1_top = y
i0 = mark()
head(LC, y+24, 'radiative-transfer model grid', 11)
...
yk = body_wrap(LX+20, y+48, 'R_{\u03bb} solves I_{\u03bb}(R_{\u03bb})/I_{\u03bb}(0) '
     '= [\u03a3(r)/\u03a3(0)]\u00b7c_{\u03bb}', LTXT, 8.4, lh=LH_DEFAULT)
...
b4_top = y
i0 = mark()
head(LC, y+24, 'Corrector  (four observables)', 11)
yk = body_wrap(LX+20, y+46, 'training set (\u03a3, \u03b6, \u03c6, R_{BE}, '
     'C_{M}=M_{BE}/M_{rec})', LTXT, 8.4, anchor='center', cx=LC)
yk = body_wrap(LX+20, yk+2, 'interpolated on log axes; the 4th axis (physical '
     'size) needs the source distance', LTXT, 7.8, MUTE, anchor='center', cx=LC, lh=15)
...
b5_top = y
i0 = mark()
head(RC, y+24, 'Real getsf catalogue', 11)
...
b6_top = y
i0 = mark()
head(RC, y+24, 'Measure 4 observables per source', 11)
yr = body_wrap(RX+24, y+48, '\u03a3 = PEAK^{BGF}  (background column)', RTXT, 8.6)
yr = body_wrap(RX+24, yr+2, '\u03b6 = PEAK^{SRC} / footprint-mean', RTXT, 8.6)
yr = body_wrap(RX+24, yr+2, '\u03c6 = A_{F} / H', RTXT, 8.6)
yr = body_wrap(RX+24, yr+2, 'footprint radius = \u221a(A_{F}\u00b7B_{F})/2, at the '
     'known distance', RTXT, 8.6)
yr = body_wrap(RX+24, yr+4, '\u03a3, \u03b6, \u03c6 are distance-invariant; the 4th '
     'needs a known distance', RTXT, 7.8, MUTE, lh=15)
...
i0 = mark()
head(MC, y+26, 'Look up C_{M}( \u03a3, \u03b6, \u03c6, size )', 11.5, PAL['key'][1])
body_wrap(MX+24, y+48, 'same corrector for every cloud, given its distance', MW-48, 8.6,
     anchor='center', cx=MC)
...
yv = bullet_wrap(VX+26, y+47, 'injection\u2013recovery: 108 synthetic cores, '
     'extracted with getsf \u2192 median M/M_{true} = 0.96, 86% within 2\u00d7',
     VW-140, 8.6)
yv = bullet_wrap(VX+26, yv+4, 'independent subdivision grid: 226 models spanning '
     'full concentration range \u2192 median M/M_{BE} = 1.01, 98% within 2\u00d7',
     VW-140, 8.6)
yv = bullet_wrap(VX+26, yv+4, 'node-level leave-one-out on the production grid '
     '\u2192 median M/M_{BE} = 1.01, 95% within 2\u00d7', VW-140, 8.6)
yv = body_wrap(VX+26, yv+6, 'none of the three used to calibrate the footprint '
     'model above', VW-52, 7.8, MUTE, lh=17)
v_bottom = yv + PAD_BOT
...
fig.savefig('/home/claude/proj/flowchart/fig_method_flowchart.pdf',
            bbox_inches='tight')
fig.savefig('/home/claude/proj/flowchart/fig_method_flowchart.png', dpi=150,
            bbox_inches='tight')
print('wrote pdf + png')
