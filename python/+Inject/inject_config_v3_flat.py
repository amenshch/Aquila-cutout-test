#!/usr/bin/env python
"""inject_config_v3_flat.py -- settings for run_inject_v3.py.

Run with:
    python -B run_inject_v3.py inject_config_v3_flat

Identical to inject_config_v3.py except that the background is flattened under
the footprint of every injected model.  Flattening removes the cloud
fluctuations beneath each core, so the pair of runs isolates the contribution
of those fluctuations to the scatter in the recovered mass: the flattened run
is the simpler case, in which the recovered mass is limited only by the
extraction and by the temperature structure of the model itself.

Note the flattening limit.  FLATTEN_MAX_FWHM_AS defaults to
OVERLAP_FREE_FWHM_AS, which is 50 arcsec, and that would silently leave the
models between 55 and 80 arcsec unflattened -- exactly the models that carry
the low-contrast end of the population.  It is therefore set explicitly to the
same value as FWHM_MAX_AS.  Interpolating the background under a footprint that
large is slower and less accurate than under a small one, so compare the two
runs in the size bins as well as in the aggregate.
"""
...
FLATTEN_MAX_FWHM_AS  = 80.0          # must match FWHM_MAX_AS; see the docstring
