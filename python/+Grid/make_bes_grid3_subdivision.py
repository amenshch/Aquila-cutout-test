...
make_bes_grid.py -- generate the pressure-truncated Bonnor-Ebert model grid
                    catalogue `bes_grid_final3.txt`.

NOTATION: a later notation pass in the paper renamed its projected-width
function from phi(xi) to varphi(xi) (freeing up the plain symbol phi for an
unrelated quantity, the footprint factor R_foot). This script's `phi` below
is the paper's varphi -- NOT its (new, unrelated) phi. Do not conflate the
two when cross-referencing paper equations against this code. Also, in the
paper's current notation: FWHM -> H (half-maximum size), R_BE is unchanged.
...
Per node the construction is:
  ...
  1. Invert (M, FWHM) -> xi_max.  For a truncated BE sphere,
     ...
     where phi(xi) = projected (column) FWHM in units of r0 [paper: varphi]
     and Cm(xi) = xi^2 dpsi/dxi is the dimensionless mass.  Both are tabulated
...
Usage
    python3 make_bes_grid.py [-o bes_grid_final3.txt]
...
FLOOR     = 1.05            # detectability floor on contrast = 1 + peak/SD_emb
...
# grid axes
SD_VALUES = [5.30e20, 1.06e21, 2.12e21, 4.24e21, 8.49e21, 1.70e22, 3.39e22, 6.79e22]       # 8 midpoints
M_MIN, M_MAX, N_M    = 0.00196, 25.12, 25                                                  # 25 midpoints
FW_MIN, FW_MAX, N_FW = 0.003376, 0.2211, 18                                                # 18 midpoints

##SD_VALUES  = [3.75e20, 7.5e20, 1.5e21, 3e21, 6e21, 1.2e22, 2.4e22, 4.8e22, 9.6e22]   # i
##M_MIN, M_MAX, N_M    = 0.0001, 100.0, 36                                             # j
...
HEADER_FMT = ("{:>4} {:>3} {:>3} {:>3} {:>9} {:>7} {:>10} {:>12} {:>8} {:>9} "
...
# Change ROW_FMT: M_BE from {:>10.4f} to {:>12.6g}, FWHM_pc from {:>8.4f} to {:>10.6g}, and widen the matching HEADER_FMT fields.

ROW_FMT = ("{:>4} {:>3} {:>3} {:>3} {:>9} {:>7.3f} {:>12.6g} {:>10.6g} {:>8.4f} "
           "{:>9.3f} {:>10.3e} {:>9.3f} {:>6d} {:>4d} {:>11.4e} {:>11.4e} "
           "{:>11.4e} {:>10.2f} {:>10.2f} {:>11.2f} {:>10.4e} {:>7.2f} {:>5}")
...
def main():
    ...
    ap.add_argument('-o', '--out', default='bes_grid_final3_subdivision.txt',
                    help='output catalogue (default: bes_grid_final3_subdivision.txt)')
    args = ap.parse_args()
