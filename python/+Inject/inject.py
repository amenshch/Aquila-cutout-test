...
def valid_placement_mask(field, sigma_cloud, R_BE_arcsec,
                         ...
                         window='symmetric', forbid_margin_as=None,
                         local_average_radius_as=None):
    """Boolean mask of pixels where the model may be centered.

    A position is valid if the whole stamp window fits inside coverage, if the
    core center is clear of `forbidden` by the required margin, and if the
    local column density is compatible with the column the model was computed
    embedded in.

    window : 'symmetric' (default) or 'one-sided'
        The embedding rungs of the Bonnor-Ebert grid, SD_emb, are spaced by a
        factor of 2, that is 0.301 in base-10 logarithm.  The symmetric window

            sigma_cloud / sqrt(2) <= local_Sigma < sigma_cloud * sqrt(2)

        is one factor of 2 wide and therefore tiles the column axis exactly:
        every column belongs to one rung, with no gap and no overlap.  The
        one-sided window used up to run_inject_v2.py,

            sigma_cloud <= local_Sigma < sigma_cloud * sqrt(2)

        is only half a rung wide, so the upper half of every octave,
        [sqrt(2)*SD_emb, 2*SD_emb), belonged to no model at all.  Half of the
        real Aquila sources sit at columns inside those gaps, including the
        median column of the population, and models drawn for them were
        silently reassigned to the rung above and placed at columns higher
        than the ones drawn.  'one-sided' is retained only to reproduce the
        earlier runs.

    forbid_margin_as : float or None
        Radius, in arcsec, by which the core center must clear `forbidden`.
        Earlier versions required the whole R_BE disk to clear it, which for a
        model of R_BE = 70 arcsec dilates the real-source footprints, covering
        6.5% of the sub-field, into an exclusion zone covering most of it.
        What has to be prevented is centering an injected core on a real
        source, not any overlap of its outer envelope, so the margin is a
        fixed length of order one beam radius rather than R_BE.  Pass None to
        require only that the central pixel itself be clear.

    local_average_radius_as : float or None
        Radius, in arcsec, over which the local column is averaged for the
        window test.  Defaults to R_BE_arcsec, the historical choice.  Pass
        the observed half-width of the model instead when R_BE is much larger
        than the size the model actually presents on the sky.
    ...
    # --- footprint avoidance, applied to the core center only ---
    if forbidden is not None and forbidden.any():
        m_pix = 0.0 if forbid_margin_as is None else forbid_margin_as / PIX_ARCSEC
        if m_pix >= 1.0:
            # A structuring element of side 2*m_pix+1 is built explicitly, so
            # the memory it needs grows as the square of the margin: at a
            # margin of 500 arcsec, about 165 pixels, the element is 331 by 331
            # and binary_erosion raised MemoryError.  A distance transform
            # answers the same question, "is every pixel within m_pix of this
            # one clear of `forbidden`", in memory proportional to the field
            # and independent of the margin, so it is used above a threshold
            # where the explicit element stops being the cheaper route.
            if m_pix > 24.0:
                from scipy.ndimage import distance_transform_edt as _dt
                fit &= (_dt(~forbidden) > m_pix)
            else:
                r = int(np.ceil(m_pix))
                yy, xx = np.ogrid[-r:r + 1, -r:r + 1]
                foot_struct = (xx ** 2 + yy ** 2) <= m_pix ** 2
                fit &= _be(~forbidden, structure=foot_struct, border_value=0)
        else:
            fit &= ~forbidden

    # --- column window ---
    # The averaging radius is the placement criterion: does this location have
    # the right ambient column?  The truth table records a larger 2*R_BE
    # average as the corrector input, computed in the driver after placement.
    R_avg_pix = (R_pix if local_average_radius_as is None
                 else local_average_radius_as / PIX_ARCSEC)
    sloc = local_sigma(field['sigma'], R_avg_pix)
    if not sqrt2_window:
        return fit
    if window == 'symmetric':
        lo = sigma_cloud / np.sqrt(2.0)
    elif window == 'one-sided':
        lo = sigma_cloud
    else:
        raise ValueError("window must be 'symmetric' or 'one-sided'")
    match = (sloc >= lo) & (sloc < sigma_cloud * np.sqrt(2.0))
    return fit & match
