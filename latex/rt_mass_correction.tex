...
fit minimizes the residual between modeled and observed band-dependent
...
center-to-edge density contrast is $\rhoc/\rho(\ximax) = 14.0$.
...
build the profile, so the inversion and the discretized profile remain
...
no recoverable size information, independent of how finely it is modeled
...
$\sim$5) were contaminated by a modeling artefact---that core's outer-radius
...
(the minimizer is clipped at the locus) and it is a result in its own
right---the method cannot characterize cores below it at a given column.
...
$\rhocloud$), the interpolator's behavior near the edge and the continuity
...
A direct comparison of model size against the published Aquila catalogue,
however, shows that the critical-sphere assumption breaks down for the more
massive cores. Convolving the model surface-density peak to the catalogue
resolution ($18.2''$, adding the difference beam in quadrature to our
$13.5''$ maps) and comparing the convolved FWHM at fixed mass, the
critical spheres match the observed core sizes below $\sim$1\,$\msun$ (ratio
$0.8$--$0.9$, real cores if anything marginally more extended), but become
progressively too large above it: the median model-to-observed size ratio is
$\approx1.3$ over 1--10\,$\msun$, and the most massive critical spheres in the
grid reach $\sim$600$''$ where no observed core exceeds $\sim$110$''$.
About a fifth of the grid (the high-mass, large-radius corner) thus occupies a
size regime absent from the data. This is expected physically: a
high-mass \emph{critical} BE sphere is necessarily large and diffuse, whereas
real massive cores are centrally condensed, consistent with external-pressure
truncation. The correction machinery developed here---recoverable mass,
concentration, source FWHM, and the completeness limit---is independent of
this and remains valid; only the underlying size template is affected. The
natural remedy, deferred to future work, is to replace the critical sphere
with a pressure-truncated (non-critical) BE family, adding the truncation
radius $\ximax<6.451$ as a grid axis so that the models populate the compact,
observed size range at each mass. We note also that the reference extraction
imposed a minimum measured size of $\sim$1.5 beams, so the observed sizes near
that floor are upper limits and the required truncation is, if anything,
somewhat stronger than the direct ratios imply.
