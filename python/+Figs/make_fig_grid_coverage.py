...
import aa_plotstyle          # shared A&A tick and font conventions
aa_plotstyle.apply()
from aa_plotstyle import clean_log, no_top_minor
...
_plasma_dark = mcolors.LinearSegmentedColormap.from_list(
    'plasma_dark', plt.cm.plasma(np.linspace(0.0, 0.88, 256)))
sc = ax.scatter(H_intr_pc, M_BE, c=np.log10(xi_max), cmap=_plasma_dark, norm=norm,
...
greys = plt.cm.Greys(np.linspace(0.95, 0.45, len(SDvals)))
for idx, SD in enumerate(SDvals):
    F = curves[SD]
    m = (F >= 1e-3) & (F <= 0.5)
    ax.plot(F[m], Mgrid[m], color=greys[idx], lw=1.3, zorder=4)
leg_line = Line2D([0], [0], color=greys[len(greys) // 2], lw=1.3)
