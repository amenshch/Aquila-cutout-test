...
We present a per-core radiative-transfer (RT) forward-modelling method
...
critical Bonnor--Ebert (BE) sphere models to the observed
...
externally heated, centrally concentrated cores are colder in the centre
...
For each core we forward-model the observable signal: a critical BE sphere
...
observational reduction. The free parameters are the core mass $M$ and the
BE temperature $\Tbe$ (equivalently $\rhoc$ and $\Rbe$), with the local
cloud column $\Scloud$ supplied per core from the observed background. The
fit minimises the residual between modelled and observed band-dependent
center intensities and FWHMs. Completeness is treated separately (via source extraction with getsf,
...
\begin{equation}
  ...
  \label{eq:r0}
\end{equation}
The critical sphere is truncated at $\ximax = 6.451$, where the
centre-to-edge density contrast is $\rhoc/\rho(\ximax) = 14.0$.

The critical BE profile is a unique self-similar shape in $\log$--$\log$:
$\Tbe$ is purely a scaling parameter that sets the physical $(M,\Rbe)$
through $\rzero$ but does not change the profile shape. This makes the
family parsimonious---there are effectively two free parameters
($M$ and $\Rbe$, bijective with $\Tbe$ and $\rhoc$ for the critical
sphere)---and is the reason a wide $\Tbe$ grid spans the observed $M$--$\Rbe$
range without introducing extra shape degrees of freedom. The use of a
single-parameter-shape template, rather than a richer geometric family, is
justified in Sect.~\ref{sec:geometry}.

\subsection{Analytic central density and mass}
\label{sec:inversion}

A critical point of method construction is that the central density must be
derived from BE physics, \emph{not} imposed by renormalising the profile to
a target mass. Renormalisation produces incorrect central densities and
...
for our table $m_{1}=C(6.451)=15.7086$, giving the mass coefficient
\begin{equation}
  \frac{m_{1}}{\sqrt{4\pi}} = \frac{15.7086}{3.544908} = 4.4313 .
  \label{eq:coeff}
\end{equation}
The inversion used to set the central density for a target mass at fixed
$\Tbe$ is then
\begin{equation}
  \rhoc = \left(\frac{4.4313\,\cs^{3}}{G^{3/2}\,M}\right)^{2}.
  \label{eq:inversion}
\end{equation}
We code the coefficient as $m_{1}/\sqrt{4\pi}$ from the same table used to
build the profile, so the inversion and the discretised profile remain
mutually consistent. \note{This single self-consistency point was the
source of an early factor-$\sim$2 mass error; keep inversion and builder
tied to one $m_1$.}
...
\subsection{Mass-conserving discretisation}
...
shell volume. Point-sampling the BE profile at cell centres does not
...
than to over-resolve the flat BE centre---without losing mass. With a
...
embedding: the core must join the cloud as an overdensity,
\begin{equation}
  \rho_{\mathrm{BE}}(\Rbe)\;\ge\;\rhocloud .
  \label{eq:contconstraint}
\end{equation}
When the BE edge density falls below the cloud density, the embedded model
is a depression (a hole), not a core, and produces a spurious negative
feature. Equation~(\ref{eq:contconstraint}) defines a minimum core mass at
each cloud column and is enforced as the low-mass edge of the grid per
column. The status of this constraint---construction self-consistency
versus physics---is discussed at length in
Sect.~\ref{sec:uniformcloud}--\ref{sec:edge}.
...
tabulated as the Black field plus a scaled hot blackbody, normalised to
...
hydrocarbon (PAH) emission bands: adding them changes core-centre
...
matches the axis to the observed local background, and relaxes
Eq.~(\ref{eq:contconstraint}) at high mass (where larger $\Rcloud$ implies
lower $\rhocloud$). The grid records $\Scloud$ (the axis) together with the
derived $\rhocloud$ and $\Rcloud$ (needed for reproducibility and for the
continuity trim, which is applied on $\rhocloud$, not $\Scloud$).
...
cavity is brighter than its centre). The interpolated background, blind to
...
\begin{itemize}
  \item the band-dependent FWHM is the temperature-gradient diagnostic
        (a colder centre is more centrally concentrated at the longer
        wavelengths);
...
$\sigma$-normalisation in the fit must be set accordingly and it must not be
weighted as a flux.}
...
The production grid is geometric (factor-2) in three axes:
$\Scloud$ (5 values), $\Tbe$ (7 values), and $\rhoc$ (8 values), with fewer
models at some nodes where Eq.~(\ref{eq:contconstraint}) excludes them.
Factor-2 spacing in $\rhoc$ at fixed $\Tbe$ corresponds to factor
$\sqrt{2}$ in $\Rbe$ and in mass ($M\propto\rhoc^{-1/2}$), i.e.\ mass nodes
$\sim$1.4 apart. We interpolate in $\log$ space on all three axes. Because
the continuity trim makes the populated node set ragged at the
low-$\rhoc$/high-$\Scloud$ corner, we use a scattered (Delaunay) interpolator
on the real node coordinates rather than a regular-grid interpolator, and we
record the continuity locus explicitly to bound the fit. \note{Omit the
trimmed models from the catalogue; if a self-describing table is wanted,
flag rows with NaN (not zero) observables.}

The mass range spans $\sim$0.01--40\,$\msun$. The low end is set by the
\emph{data} resolution: below the $3''$ map grid and the beam a core carries
no recoverable size information, independent of how finely it is modelled
(Sect.~\ref{sec:images}). The high end is capped above the observed Aquila
maximum ($\sim$20\,$\msun$) with a fitting margin, rather than at the
mathematical limit of the BE family (Sect.~\ref{sec:floorcap}).
...
finely it is modelled. The model floor and the data floor therefore diverge,
...
stated explicitly so that ``modelled down to mass $X$'' is not read as
...
interpolated grid. The residual vector is the difference between modelled
and observed observables (Sect.~\ref{sec:observables}), normalised by a
...
continuity locus so the minimiser cannot enter the unpopulated corner.
...
neighbouring columns). The failure is not a radiative-transfer or arithmetic
...
RADMC-3D and omit PAH. \note{An early alarming 6-vs-12~K core-centre
...
modelled cloud surface is 16.3~K, the centre next to the core is 8.9~K, and
...
With the discretisation of Sect.~\ref{sec:massconserv} and a wall snapped to
...
For a continuous $0.1\,\msun$ core in a $2\times10^{22}\,\mathrm{cm^{-2}}$
cloud at $G_{0}=10$:
\begin{center}
\begin{tabular}{lcc}
\hline
Background scheme & $\Tsed$ (K) & $M$ ($\msun$)\\
\hline
Crater (core removed)    & 8.52 & 0.1015 \\
Interpolated             & 8.26 & 0.0676 \\
\hline
\end{tabular}
\end{center}
The crater scheme recovers the true mass and temperature; the interpolated
scheme is cold-biased and under-estimates the mass by $\sim$1.5. The
350\,$\mu$m peak is a factor $\sim$2 lower for the interpolated case,
because it subtracts the edge cloud value across the footprint while the
actual cloud under the core (the crater) is lower. The arithmetic closes:
with $M\propto F/B_\nu(T)$, a flux ratio $\sim$0.5 and
$B_\nu(8.52)/B_\nu(8.26)\approx 1.16$ at 350\,$\mu$m give $\sim$0.58, versus
the observed $0.0676/0.1015=0.67$ (approximate, since the mass comes from
the full multi-band fit, not the single 350\,$\mu$m peak).

\note{SUPERSEDED, do not reuse: earlier $0.05\,\msun$ numbers
(interpolated 0.0178, crater 0.0507, mass factor $\sim$2.8, peak factor
$\sim$5) were contaminated by a modelling artefact---that core's outer-radius
density dipped below the cloud density, creating a spurious depression. The
clean effect at this mass/column is factor $\sim$2 in peak, $\sim$1.5 in
mass; it may grow at higher columns and lower masses, to be mapped rather
than asserted.}
...
background-subtracted colour temperature is $11.97$\,K and the recovered mass
...
grid's behaviour at high column directly checkable against published results.
...
constructed; left unmodelled, it drives the corrected-to-true mass ratio down
...
---cannot expand past the radius where the source profile sinks into the
cirrus fluctuations, so the effective footprint is
$\min(\eta\,H_n,\,r_{\mathrm{noise}})$, with $r_{\mathrm{noise}}$ the radius at
which the azimuthally averaged profile falls to $5\,N_{\mathrm{rms}}$. Second,
the residual background pedestal at that rim is subtracted over the footprint.
...
which replaces the idealised model mass as the interpolation coordinate. In
...
modelling (the band-dependent FWHMs constrain the gradient); the
...
\subsection{The uniform-cloud idealisation}
...
\paragraph{Construction self-consistency, not a physical law.}
The continuity requirement $\rho_{\mathrm{BE}}(\Rbe)\ge\rhocloud$
(Eq.~\ref{eq:contconstraint}) is a property of the uniform-cloud geometry:
the local volume density at the core boundary \emph{is} $\rhocloud$ in the
model, so the requirement that the core be an overdensity is a
self-consistency condition for this particular embedding, not a statement
about which cores can exist. Physically, a core's local volume density and
its observed surface density are decoupled---the column integrates the
\emph{entire} cloud along the sightline, while the local density is set by
where the core sits in the cloud's three-dimensional structure---so real
cores can certainly exist at masses the uniform-cloud grid excludes.

\paragraph{The edge is an applicability boundary, not a mass bias.}
Crucially, the arbitrariness lives entirely at the boundary. Inside the
populated region the construction is self-consistent and mass recovery
stands on its own; the continuity edge does not propagate inward as a mass
error. It marks where the uniform-cloud model applies, not where cores
exist---a distinction we state explicitly so the edge is not misread as a
formation limit (Sect.~\ref{sec:edge}).
...
assumptions with explicit, testable ones (a BE profile, a forward-modelled
background, a parameterised cloud) is methodologically preferable even when
the explicit model is itself idealised. The honest claim is ``less biased
than the standard, with a stated and quantified residual,'' and that claim
is robust to the cloud idealisation.
...
grid. If the recovered mass shifts within the error budget, the idealisation
...
Per $\Scloud$ node, Eq.~(\ref{eq:contconstraint}) defines a minimum
recoverable core mass. We carry this locus explicitly: it bounds the fit
(the minimiser is clipped at the locus) and it is a result in its own
right---the method cannot characterise cores below it at a given column.
Because the \texttt{SD} (surface-density) observable is the quantity most
directly tied to the continuity edge (it is where $\rho_{\mathrm{BE}}$ meets
$\rhocloud$), the interpolator's behaviour near the edge and the continuity
...
uniform-cloud model is silent, which is not a claim of absence.
...
\begin{figure}
  \centering
  \includegraphics[width=\hsize]{completeness_limit.pdf}
...
We adopt critical BE spheres rather than a richer geometric family
(non-spherical, multiple shape parameters) for two reasons. First,
parsimony: the critical BE profile is a single self-similar shape, so the
family has effectively two parameters. Second, resolution: convolution to
the \emph{Herschel} beams washes out shape differences at the
$\sim$5\% level, below the noise, so geometric detail is not recoverable and
its inclusion would add unconstrained parameters. The resolution argument is
the defensible justification; no dedicated geometric-sensitivity test is
required, though one can be quoted.
...
We have defined a per-core RT forward-modelling method that corrects the two
...
analytically derived central densities, mass-conserving discretisation, a
wall snapped to $\Rbe$, and a continuity-constrained low-mass edge; the
...
\begin{itemize}
  \item \textbf{Paper~I (method).} BE construction, RT calibration, the two
        ...
        Heavy numerics ($m_{1}$, mass conservation, discretisation) in an
        appendix.
